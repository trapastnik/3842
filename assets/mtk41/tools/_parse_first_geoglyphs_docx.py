...
def guess_kind(item):
    ...
    Судим по size_raw и title, но НЕ по short_text: в примечаниях «барельеф»
    сплошь и рядом — деталь постамента или предыстория места («первоначально
    здесь был барельеф»), а не сам объект. Внутри size_raw порядок проверок
    тоже важен: у Костромы «общая высота памятника 23,5 м, высота скульптуры
    8 м» — это скульптура, хотя слово «барельеф» встречается рядом.
    """
    blob = f"{item.get('size_raw','')} {item.get('title','')}".lower()
    if 'геоглиф' in blob:
        return 'geoglyph'
    if 'мемориальн' in blob and 'плит' in blob:
        return 'plaque'
    # «фигур» отдельно не берём: в Регле это «бюст Ленина и 12 мраморных фигур»
    # — фигуры там свита, а объект всё-таки бюст. Нужен именно оборот
    # «высота фигуры».
    if re.search(r'скульптур|стату\w|голова|высота фигуры|фигура высотой|фигуры высотой', blob):
        return 'sculpture'
    if 'бюст' in blob:
        return 'bust'
    if re.search(r'барельеф|горельеф|стела', blob):
        return 'relief'
    # По short_text не гадаем: там биография МЕСТА, и «бюст» чаще относится к
    # предшественнику («первый памятник 1925 г. — небольшой бронзовый бюст»),
    # а не к описываемому объекту. Молчит size_raw — считаем скульптурой.
    return 'sculpture'
