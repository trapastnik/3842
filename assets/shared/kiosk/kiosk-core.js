...
(function (global) {
  ...
  KioskApp.prototype.registerScene = function (def) {
    ...
    /* ОБЪЯВЛЕНО, НО НЕ РЕАЛИЗОВАНО — тихий класс (находка МТК 42).
     *
     * safeCall не считает отсутствие метода ошибкой: отбор в ядре идёт,
     * точка на лупе горит, журнал чист — а экран стоит. Глазами это
     * неотличимо от «фильтр не нашёл ничего по данным», и искать причину
     * будут в данных.
     * Проверяем ОБА направления: декларация без метода (отбор некому
     * применить) и метод без декларации (его никто не позовёт). Второе
     * тише первого — там даже точки не будет.
     * Момент выбран самый ранний: на регистрации, а не при первом отборе, —
     * иначе предупреждение ждало бы посетителя. */
    var hasSpec = !!(def.finder && typeof def.finder === "object");
    var hasApply = typeof def.applyFinder === "function";
    if (hasSpec !== hasApply) {
      var msg = hasSpec
        ? "сцена «" + def.id + "» объявила finder, но не реализовала applyFinder — " +
          "отбор будет считаться (точка на лупе загорится), а экран не изменится"
        : "сцена «" + def.id + "» реализовала applyFinder, но не объявила finder — " +
          "метод не будет вызван никогда, иконка поиска не появится";
      console.warn("[kiosk] " + msg);
      this.log("warn", msg, { scene: def.id });
    }
