 ...
 *   4. Поиск финдера не отключён нехваткой высоты.
 *
 * ОГОВОРКА к пункту 4: он смотрит АКТИВНУЮ сцену. У сцены без finder{} или
 * без search проверка молча выпадает — прогоняйте на той сцене, где поиск
 * объявлен, иначе строка «сломано N» про поиск ничего не говорит.
 *
...
(function (global) {
  ...
  function run(app, opts) {
    ...
    function check() {
      ...
      /* Вмещаемость измерима только на ОТКРЫТОЙ панели — открываем сами и
       * возвращаем как было; иначе получили бы отказ «не мерил». */
      if (typeof app.finderSearchFits === "function" && app.finderSpec && app.finderSpec()) {
        var былаОткрыта = !!app._finderOpen;
        if (!былаОткрыта) app.openFinder(true);
        var f = app.finderSearchFits();
        if (!былаОткрыта) app.openFinder(false);
        if (f && f.fits === false && f.need) {
