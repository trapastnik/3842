...
4. **Скроллы видимые.** Каждому скроллируемому контейнеру — `class="kiosk-scroll"`.
   ...
   **Один контейнер — один владелец подсказки.** Повторный `attach` на тот же
   контейнер обновляет существующую подсказку, а не создаёт вторую: если две
   сцены с `keepAlive` делят общий холст (как у МТК 38), они будут спорить за
   подпись, и `destroy()` одной снимет подсказку у обеих. Нужны разные — давайте
   разные контейнеры.
