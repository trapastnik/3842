...
<html lang="ru">
  <head>
    ...
    <title>МТК 42</title>
    <link rel="stylesheet" href="../assets/shared/kiosk/kiosk.css?v=1.23.0" />
    <link rel="stylesheet" href="../assets/shared/kiosk/kiosk-core.css?v=1.23.0" />
    <link rel="stylesheet" href="./styles.css?v=1.23.0" />
  </head>
  <body>
    <script type="module" src="./app.js?v=1.23.0"></script>
  </body>
</html>
```
...
import { createApp } from "../assets/shared/kiosk/kiosk-core.esm.js?v=1.23.0";
import { mapScene } from "./scenes/map.js?v=1.23.0";        // импорты сцен тоже!
import { timelineScene } from "./scenes/timeline.js?v=1.23.0";
...
const app = createApp({
  ...
  i18nUrl: "./i18n/",
  i18nVersion: "1.23.0",                 // метка кеша словарей — та же версия ядра
});
...
```html
<script src="../assets/shared/kiosk/kiosk-core.js?v=1.23.0"></script>
<script>
...
2. **Верстайтесь от зон хрома.** Ядро объявляет, сколько места заняли навигация,
   ...
   **Тесное поле — громкий сигнал (с 1.23.0).** Даже когда зоны НЕ ужаты, поле
   может выйти тесным: хром съедает больше половины оси. На 1080 поле 632 при
   хроме 448 — норма (молчим); на 720 поле 236 при хроме 484 — теснота. Кит
   пишет об этом в консоль ОДИН раз на вход в тесноту и ставит на корень
   атрибут `data-kiosk-field-cramped`; хост волен оформить это состояние (ядро
   само ничего не рисует поверх сцены). Причина обычно не в сцене, а в том, что
   вьюпорт ниже киоск-спеки (штатно 1080) или крупный масштаб интерфейса:
   абсолютные тач-таргеты на низком окне не оставляют сцене пропорции. Порог без
   магии — «поле меньше съевшего его хрома», при снятии тесноты сигнал
   перевооружается. Отдельно от ужатия выше: у того свой, более точный warn, и
   когда сработал он — этот молчит.
...
4. **Скроллы видимые.** Каждому скроллируемому контейнеру — `class="kiosk-scroll"`.
   Он же получает `touch-action: pan-y` — палец скроллит контейнер, а не зумит
   страницу.

   **Зум — контента сцены, а не страницы.** Браузерный зум/скролл всей страницы
   ядро подавляет (киоск-канон): `visualViewport.scale` всегда 1, двойной тап и
   Ctrl-колесо нативно не зумят. Но жест у вас **не отнят** — он ваш:

   - канва внутри `.kiosk-stage` по умолчанию получает `touch-action: none`.
     Это значит, что браузер по ней **ничего не делает сам**, и весь жест —
     и одно-пальцевый drag, и двухпальцевый пинч — уходит в ваши
     `touchstart/touchmove/pointermove` слушатели. Считайте пинч по
   ...
   **`hint.js` — отдельный скрипт со своей меткой кеша**, и проверка версий его
   ...
   ```html
   <script src="../assets/shared/kiosk/hint.js?v=1.23.0"></script>
   ```
...
`<script src="./app.js?v=1.23.0">` обновит только сам `app.js`. Его
...
адреса, так что `import … from "…/kiosk-core.esm.js?v=1.23.0"` загрузит и
`kiosk-core.js?v=1.23.0`. Версия ядра поднимается в одном месте. **У ваших сцен —
нет:** тут думать вам.
...
```html
<script src="../assets/shared/kiosk/tools/selftest.js?v=1.23.0"></script>
<script src="../assets/shared/kiosk/tools/audit-settings.js?v=1.23.0"></script>
```
