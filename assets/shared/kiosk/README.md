...
4. **Скроллы видимые.** Каждому скроллируемому контейнеру — `class="kiosk-scroll"`.
   ...
   **Один контейнер — один владелец подсказки.** Повторный `attach` на тот же
   контейнер обновляет существующую подсказку, а не создаёт вторую. Спор
   возникает, только если два владельца в момент `attach` дают **один и тот же
   элемент** — например, обе сцены передали общий холст, лежащий в общем
   родителе. Тогда они делят подпись, и `destroy()` одной снимет подсказку у
   обеих. Разные контейнеры — разные подсказки; общий холст, лежащий у каждой
   сцены в своём корне, спора не создаёт.
...
поднялось. Поэтому сверка версий встроена: залипший кеш ловится кодом, а не
глазами в отчёте. Расхождение пишется в консоль и в журнал.
...
по определению, у версий до 1.9.0 её просто нет. Обёртка же всегда свежая — её
адрес и несёт бампнутую метку.
