...
# Атрибуция по российскому законодательству — решение пользователя 2026-08-03.
# Города, вошедшие в состав РФ, числятся за Россией: так же, как уже отрисованы
# границы на «Карте Союза». В музейных материалах эти записи стоят за Украиной,
# и исходная графа не теряется — она уходит в `country_src`. По ней geocode.py
# и строит запрос, и сверяет точку с контуром страны: иначе разовый геокодинг
# перестал бы воспроизводиться (кеш по строке запроса) и выбросил бы точки как
# «не попавшие в свою страну». Идентификатор записи тоже остаётся прежним —
# он собран из графы материалов, к нему привязаны имена файлов снимков и
# кураторская выборка.
RU_LAW_CITY = re.compile(
    r'^(Запорожь|Донецк|Горловк|Краматорск|Херсон|Мариупол|Луганск|Макеевк|'
    r'Мелитопол|Бердянск|Энергодар|Северодонецк|Алчевск|Енакиев|Дебальцев)', re.I)


def by_russian_law(city, country):
    """Страна записи с учётом действующих границ РФ; вторым значением — исходная."""
    if (country or '').strip() == 'Украина' and RU_LAW_CITY.match((city or '').strip()):
        return 'Россия', country
    return country, None
...
def build():
    ...
    for src, fname in FILES:
        for section, cells in walk(os.path.join(SRC, fname)):
            ...
            # id уже собран — атрибуция меняется после него, чтобы не поехали
            # имена файлов снимков и ссылки кураторской выборки
            country, country_src = by_russian_law(city, country)
