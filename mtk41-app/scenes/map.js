...
import { createMapCore } from "./map-core.js?v=67";
...
export const mapScene = {
  ...
  /* ДВА ПЕРЕВОДА, И ЭТО НЕ ДУБЛИРОВАНИЕ.
   *
   * Сцена рисует в двух системах координат, и путать их — ровно тот дефект,
   * который ломал карту дважды подряд:
   *
   *   _worldToStage  — для того, что рисуется ВНУТРИ ctx.scale(zoom): контуры
   *     стран. Зум там даёт сам холст, и переводу его добавлять нельзя.
   *   _pointToScreen — для того, что рисуется СНАРУЖИ трансформа: кружки
   *     кластеров. Их радиус задан в экранных пикселях и зумом расти не
   *     должен, поэтому холст их не масштабирует — зум обязан быть в переводе.
   *
   * История обеих поломок, чтобы третьей не было. Сперва перевод был БЕЗ зума
   * — и кружки, рисуемые снаружи, оставались в предзумных координатах:
   * расхождение с картой 57 px при зуме 0.75 и 756 px при 6, кружки кучей у
   * центра (это и увидел пользователь). Потом я сделал перевод
   * зум-осведомлённым ГЛОБАЛЬНО — кружки встали на место, но _drawBase зовёт
   * тот же перевод изнутри ctx.scale, и страны поехали в зум²: медиана
   * расхождения 44 px на дефолтном виде и 866 px на зуме 3. Ошибка не
   * исчезла, а переехала с кружков на страны — поймано скептиком.
   *
   * Поэтому _pointToScreen ВЫРАЖЕН через _worldToStage: он буквально
   * повторяет то, что делает с точкой ctx.scale(zoom) вокруг центра стола.
   * Разойтись эти двое теперь не могут — вторая формула не переписывается
   * независимо от первой, а composition остаётся обратной к _clientToWorld. */
  _worldToStage(wx, wy) {
  ...
  _pointToScreen(wx, wy) {
    ...
    const s = this._worldToStage(wx, wy);
    return {
      x: (s.x - h.width * 0.5) * m.zoom + h.width * 0.5,
      y: (s.y - h.height * 0.5) * m.zoom + h.height * 0.5,
    };
  },
      ...
      for (const poly of polys) {
        for (const ring of poly) {
          ...
          for (let i = 0; i < ring.length; i += 1) {
            ...
            /* _worldToStage, НЕ _pointToScreen: мы внутри ctx.scale(zoom),
             * зум уже даёт холст. Зум-осведомлённый перевод здесь дал бы
             * зум² — страны уезжали бы от кружков тем сильнее, чем ближе
             * приближение. Соседняя компенсация lineWidth (invZ = 1/zoom)
             * — свидетель того же: мы под масштабом. */
            const s = this._worldToStage(p.x, p.y);
