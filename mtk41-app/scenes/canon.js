...
export const canonScene = {
  ...
  standby() {
    ...
    const step = (t) => {
      /* Гасим призыв к жесту НА КАЖДОМ тике, а не один раз на входе.
       * У кита нет способа сказать «держи погашенной, пока идёт заставка»:
       * poke() перевзводит одноразовый таймер показа на 30 с, и после одного
       * вызова подсказка через полминуты загоралась поверх фотографии и висела
       * так до утра (замер скептика: t=105 с, opacity 1, z 40 над заставкой
       * z 15). Тик петли короче 30 с, поэтому таймер просто никогда не
       * дотикает. Обход до появления suppress()/resume() в ките — заявка
       * подана; как появятся, эта строка меняется на них.
       *
       * У обхода есть цена, и её стоит знать: он верен, ПОКА такт тикера
       * короче 30 с. При штатном standbyFps это с огромным запасом (замер:
       * 53 вызова за 53 с, подсказка тёмная на 74-й секунде — порог пройден
       * вдвое). Но поставь оператор в сервисной панели совсем медленный
       * standbyFps — и подсказка вернётся посреди заставки. Настоящий
       * suppress() такой связи не имел бы; это и есть довод в заявку. */
    ...
    step(0);
    /* Гасим призыв к жесту на входе — и здесь, и в каждом тике (см. step).
     * Здесь потому, что первый колбэк тикера приходит через ЦЕЛЫЙ период
     * после старта, и без этого вызова подсказка успела бы мелькнуть поверх
     * первого кадра заставки. В тике — потому что один вызов её не удержит.
     *
     * На выходе перевзводим: одноразовый таймер простоя к этому моменту уже
     * отработал, без rearm() подсказка не вернулась бы до перезапуска киоска. */
    if (this._hint) this._hint.poke();
