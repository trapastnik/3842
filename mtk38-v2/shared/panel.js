...
*/
(function () {
  // — авто-подключение panel.css рядом с этим скриптом (если ещё не подключён) —
  (function ensureCss() {
  ...
  function create(opts) {
    ...
    var state = {}, defaults = {};
    var valEls = {}, inputEls = {}, togEls = {}, segEls = {}, grpDefs = [], segDefs = [];
    ...
    // сегменты — вверху (композиция/режим)
    segments.forEach(function (s) {
      ...
      wrap.appendChild(btns); body.appendChild(wrap);
    ...
    groups.forEach(function (g, gi) {
      ...
      (g.params || []).forEach(function (p) {
        ...
        row.appendChild(inp); ge.appendChild(row);
      });
