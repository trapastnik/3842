...
/** Погасить подсказку жеста и перевзвести её 30-секундный цикл.
 *
 * Зовётся ПЕРВОЙ строкой каждого resume(), до любых ранних выходов. Причина
 * тонкая: подсказка живёт у сцены постоянно (сцены keepAlive), и пока сцена
 * ждала своей очереди неактивной, её таймер простоя спокойно отстрелялся —
 * к моменту показа она уже горит. Ротация заставки показывает сцену за
 * ~350 мс до того, как позовёт её standby(), и всё это окно призыв «коснитесь»
 * висит поверх аттрактора; в интерактиве он точно так же вспыхивает сразу
 * после касания навигации, хотя посетитель только что показал, что жест знает.
 * resume() — единственная точка, которая срабатывает ДО кроссфейда.
 *
 * poke(), а не hide(): у кита это «отметить взаимодействие» — гасит, если
 * горит, и в обоих случаях заводит цикл заново. */
export function hushHint(hint) {
  if (hint && hint.poke) hint.poke();
}

/** Повесить подсказку жеста так, чтобы она не вспыхнула поверх заставки.
 *
 * Кит держит ДВА таймера: цикл простоя (idleMs) и одноразовый первый показ
 * (firstDelay, 1.2 с — «не мигать во время загрузки»). poke()/hide() перевзводят
 * только первый; firstDelay стреляет при любом раскладе. В ротации заставки
 * сцена монтируется прямо на глазах у зала — и через 1.2 с призыв «коснитесь»
 * загорается поверх аттрактора. Тик заставки его гасит, но между тиком (100 мс)
 * и CSS-затуханием (0.5 с) выходит видимое мигание — замерено на первой смене
 * сцены. Если монтируемся в заставке, первого показа просто не назначаем. */
export function attachHint(app, root, opts) {
  if (!window.KioskHint || !root) return null;
  const o = opts || {};
  const idleMs = o.idleMs || 30000;
  const standby = !!(app && app.idleState === "standby");
  return window.KioskHint.attach(root, Object.assign({}, o, {
    idleMs,
    firstDelay: standby ? idleMs : o.firstDelay || 1200,
  }));
}
 ...
 * `hintOf` — доступ к подсказке жеста сцены; см. ниже, зачем её дёргать.
 */
export function beginStandby(app, tick, cleanup, hintOf) {
  let busy = false;
  const draw = () => {
    /* Подсказку гасим В КАЖДОМ ТИКЕ, а не один раз на входе.
     * У кита показ висит на одноразовом таймере бездействия (30 с), и в
     * заставке бездействие вечное: одиночное гашение означает, что призыв
     * «коснитесь, чтобы…» загорится через полминуты ПОВЕРХ аттрактора и будет
     * гореть до утра. poke() гасит показанную и перевзводит таймер, который до
     * следующего тика не доживает. Обход до suppress()/resume() в ките. */
    const hint = hintOf && hintOf();
    if (hint && hint.poke) hint.poke();
  ...
  /* Гасим сразу, не дожидаясь первого тика: ротация показывает сцену через
   * showScene, ядро на показе зовёт resume — и в окно между показом и этим
   * вызовом (кроссфейд ~350 мс) подсказка успевает загореться, потому что её
   * таймер взведён ещё с прошлого показа сцены. Поймано наблюдением: ровно
   * одна вспышка на смене globe→rain. */
  {
    const hint = hintOf && hintOf();
    if (hint && hint.poke) hint.poke();
  }
