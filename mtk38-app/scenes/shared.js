 ...
 *
 * Зовётся ПЕРВОЙ строкой каждого resume(), до любых ранних выходов.
 *
 * Это НЕ про заставку — там с 1.13.0 всё делает подавление ядра. Случай другой
 * и остаётся живым: сцены keepAlive, подсказка живёт у сцены постоянно, и пока
 * сцена ждала своей очереди неактивной, её таймер простоя спокойно отстрелялся.
 * Посетитель жмёт стрелку навигации — и получает готовый призыв «коснитесь» на
 * первом же кадре новой сцены, хотя только что показал, что управляться умеет.
 * Касание пришлось на бар навигации, то есть вне контейнера сцены, и штатный
 * слушатель кита цикл не перевзвёл.
 *
 * Проверено контрольным опытом на 1.14.1 при idleState = active: подсказка
 * неактивной сцены показана через hint.show(), затем сцена выведена на экран —
 * без этого вызова она остаётся гореть, с ним гаснет.
 *
