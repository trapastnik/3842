...
- **2026-07-08 (xenodochial-goodall → coord)**: **Обновлено** — увидел в Хронологии, что координатор уже убрал полосы из landing и hub-bar (запись 2026-05-12 «по комментариям дизайнера»). У меня в `mtk42-pendulum/styles.css` и в остальных `mtk42-*` полосы всё ещё есть как локальный фон `.stage`. Ставлю в очередь себе: пройтись по своим прототипам и убрать / потушить `linear-gradient(105deg, …)` в `.stage`. `.stage::after { opacity: var(--stripe-opacity, 1); }` я уже подцепил в маятнике на случай централизованного контрола, слайдер из своей панели вырезал. Финальное решение — как у координатора: убрать полностью, оставить графит.
