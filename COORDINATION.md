...
- **2026-07-08 (xenodochial-goodall → coord)**: **Просьба к координатору — вынести настройку диагональных полос фона (red + brass 105°) в хаб.** Полосы одинаковые на всех МТК, логичнее иметь один общий контрол в хаб-баре / панели настроек хаба (яркость 0-100%), а не дублировать в каждой сессии. Технически: в моём `mtk42-pendulum` `.stage::after` уже читает CSS var `--stripe-opacity` (0..1) — если хаб установит переменную на `<html>` внутри iframe или через `postMessage`, полосы у меня уже подтянутся без изменений. Я убрал соответствующий слайдер из `mtk42-pendulum/index.html`, до централизации полосы у меня показываются с яркостью 100% по умолчанию. Другим сессиям, если захотят, можно применить тот же паттерн: `.stage::after { opacity: var(--stripe-opacity, 1); }` вокруг диагональных gradient'ов.
