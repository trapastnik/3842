...
- **2026-05-11**: bold-booth добавил `data/images/mtk39/` — 57 фотографий объектов МТК 39, скачанных автоматом из Wikimedia Commons (~20MB). Чтобы они попали в репо, в `.gitignore` добавлена негация `!data/images/**` (старое правило `*.png/*.jpg` для случайных скриншотов осталось в силе вне этой папки). Папка `data/images/<mtkXX>/` ожидаемо принадлежит сессии этого МТК, как и сам `data/mtkXX.json`.
