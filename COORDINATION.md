...
- **2026-07-07 (coord, настройки хаба)**: В хаб-баре появилась **шестерёнка ⚙ → выезжающая справа панель настроек** (общий `assets/shared/hub/`, на всех МТК). Группа «Стрелки навигации» (положение по вертикали/горизонтали, яркость, размер — CSS-переменные `--nav-*` в документе хаба). Группа «Полосы на фоне» — **каждая полоса управляется отдельно**: хаб ставит `--stripe-red-opacity` и `--stripe-brass-opacity` (обе 0..1) **внутрь iframe** прототипа (same-origin) и переприменяет при каждой смене варианта.
  **Конвенция для сессий:** если у прототипа есть брендовые фоновые полосы (красная широкая + латунная тонкая диагонали) — заведи **альфу каждой полосы на свою переменную**, и хаб будет крутить их по отдельности; своя секция настроек полос в прототипе не нужна. Прототипы без полос переменные игнорируют. Сохраняется в localStorage (`bmk-hub-nav`, `bmk-hub-stripe`). Пример миграции CSS (relative color syntax, поддержка ≥ Safari 16.4 / Chrome 111):
  ```css
  .stage::after {
    background:
      linear-gradient(105deg, transparent 72%,
        rgb(from var(--red)   r g b / var(--stripe-red-opacity, 1))   72%,
        rgb(from var(--red)   r g b / var(--stripe-red-opacity, 1))   86%, transparent 86.2%),
      linear-gradient(105deg, transparent 62%,
        rgb(from var(--brass) r g b / var(--stripe-brass-opacity, 1)) 62%,
        rgb(from var(--brass) r g b / var(--stripe-brass-opacity, 1)) 62.18%, transparent 62.36%);
    /* убрать общий opacity: var(--stripe-opacity) — теперь альфа в самих цветах */
  }
  ```
  **Задача МТК 42:** мигрировать `.stage::after` в `mtk42-*` с одного `--stripe-opacity` на две переменные (сниппет выше). До миграции хаб-слайдеры инертны (переменные ставятся, но старый CSS их не читает) — не ломается, просто ждёт.
