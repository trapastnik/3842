...
**Вопросы к координатору** (детали в HANDOFF):
1. Объём языков v2 — 53 (по PDF) / ~80 / 134 (планка ПСС Ленина)?
2. Разрешаем ли **WebGL** для арт-качества на 4K? Это правка общего правила «без WebGL» в `CLAUDE.md` — менять может только координатор.
3. Место для **bundled-шрифтов Noto** (нужны для не-латиницы, иначе квадраты): оставить `mtk38-globe/fonts/` или завести `assets/shared/fonts/`?
4. Вендоринг `pretext` в `mtk38-poster` (пункт миграции выше) — закрыть в v2.
