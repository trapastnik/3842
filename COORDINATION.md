...
**Решение — вариант A: бинарный контент лежит в git, в централизованной папке `assets/mtkXX/`.** Деплой = `git pull` на VPS, никаких дополнительных шагов. Альтернативы (Git LFS, внешнее хранилище / S3) — преждевременная сложность; на нынешних объёмах (десятки МБ на МТК) git справится спокойно.

### Где лежит

```
assets/
├── mtk38/     ← владелец: mtk38-worker
├── mtk39/     ← владелец: bold-booth-f49a93
├── mtk40/     ← владелец: flamboyant-cartwright-839223
├── mtk41/     ← владелец: reverent-chandrasekhar-3484a8
├── mtk42/     ← владелец: xenodochial-goodall-8fcd6a
└── shared/    ← кросс-МТК ассеты (если появятся). Менять только через координатора.
```

Структура внутри `assets/mtkXX/` — на усмотрение владельца. Разумные конвенции:

- `assets/mtkXX/<категория>/<slug>.<ext>` — например `assets/mtk42/portraits/lenin-1924.jpg`
- `assets/mtkXX/<объект>/<файлы>` — например `assets/mtk41/volgograd-1973-vuchetich/photo.jpg` (как у МТК 41 сейчас)
- `assets/mtkXX/manifest.json` — опционально, если нужен список с метаданными (автор, дата, лицензия, источник).
- `assets/mtkXX/tools/` — скрипты подготовки контента (Python и т.п.) тоже в git. `__pycache__/` исключён в `.gitignore`.

### Что разрешено в git

- `png`, `jpg`, `jpeg`, `gif`, `webp` — **только под `assets/`**. Глобально вне `assets/` они в `.gitignore` чтобы случайные скриншоты рабочей среды не попадали в репо.
