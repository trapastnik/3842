...
import { DATA, STATUS_COLOR, museumCardHtml, createOverlay, esc } from "./shared.js?v=29";
...
export const mapScene = {
  ...
  _liftHint() {
    ...
    /* Стык с scopeTo ядра (капкан координатора, 1.20.3) разобран по трём
     * рёбрам, все чистые:
     *  — сцена активируется уже в тесноте: класс ставит наблюдатель на баре в
     *    первом же кадре, а первый показ отложен на firstDelay 1.2 с — призыв
     *    невидим с рождения, мигания «показался и погас» нет;
     *  — выход из тесноты при горящем цикле: is-on не сбрасывается, снятие
     *    класса открывает призыв в ТОТ ЖЕ цикл (без плавного проявления —
     *    переход отыграл, пока пилюля была скрыта);
     *  — вход в тесноту при горящем призыве: гаснет мгновенно и чисто, потому
     *    что transition-property у пилюли — только opacity и transform,
     *    visibility в нём нет, полупрозрачного зависания не бывает.
     * Остаётся одно окно: между ресайзом и доставкой наблюдателя призыв виден
     * и налезает на заголовок. На живом экране это один кадр. */
