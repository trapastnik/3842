...
export function installChromeZones(app) {
  /* Замер СРАЗУ и ещё раз следующим кадром — оба пути нужны, и вот почему.
   *
   * Только rAF нельзя: в неактивной вкладке кадры заморожены, и первый замер
   * не случился бы вовсе (ловушка, на которой спотыкалось ядро 1.0).
   * Только setTimeout нельзя тоже, а именно так здесь и было: макрозадача в
   * скрытой вкладке душится до одного раза в минуту, и при переключении в
   * режим слабовидящих зона оставалась старой. Стоило это дорого: навигация
   * в a11y растёт с 198 до 242 px и занимает снизу 322 вместо 302, а поле
   * канвы держалось прежних 302 — то есть двадцатью пикселями заходило ПОД
   * бар навигации. Нашлось при переезде на кит 1.12.0: подсказка встала на
   * 51 px вместо обещанных 15, и разница оказалась ровно нехваткой зоны.
   * Синхронный замер даёт верное число сразу (getBoundingClientRect и так
   * форсирует раскладку), кадровый — страхует случай, когда перелайаут
   * материализуется только следующим кадром (сага 42). */
  let timer = 0;
  const schedule = () => {
    measureChrome();
    if (timer) return;
    timer = requestAnimationFrame(() => { timer = 0; measureChrome(); });
  };
