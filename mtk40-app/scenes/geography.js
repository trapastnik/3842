...
export const geographyScene = {
   ...
   *
   * ПОЗИЦИЯ — ПО ДВУМ СЛУЧАЯМ, и это доводка по проду (регресс зум-волны).
   * При zf > 1 (посетитель ПРИБЛИЗИЛ) держим мировую точку под центром кадра:
   * при смене k с фиксированными ox/oy она уезжает на (k_new−k_old)·доля — на
   * a11y стол ужимается 3-5%, композиция дёргалась вбок (замер: 96 px на zf1,
   * 332 на zf3). Запоминаем точку под центром ДО смены k, применяем новый k
   * ВОКРУГ неё (_placeAt-от-центра, как onWheel вокруг курсора).
   * При zf ≈ 1 (ВСЯ КАРТА, посетитель не приближал) держать точку НЕЛЬЗЯ —
   * надо центрировать по ГОРОДАМ. Первая версия держала точку всегда, и при
   * БОЛЬШОЙ смене стола (не киосочный resize 3840→480) города уезжали за край:
   * 1 из 16 в кадре. На фикс. размере киоска это не всплывало (свежий показ
   * идёт через fitToCities), но нашлось прод-проверкой на узком вьюпорте.
   * Порог 1.001: строгое zf==1 не переживает float. */
  recomputeFit() {
    ...
    if (this.fitted) {
      ...
      if (zf <= 1.001) {
        /* вся карта — центр по городам, не по прежней точке */
