...
/* ------------------------------------------------------------------ поиск
 * Сопоставитель запроса для финдера. Один на сцены, объявившие search, —
 * иначе «Ленин» находил бы разное в картотеке и на карте.
 *
 * Нормализация нужна не для красоты, а потому что в корпусе:
 *  - ё и е соседствуют («Чернышевский» против «Что дѣлать»-подобных форм и
 *    живого «ё» в «Плеханов о Чернышёвском»);
 *  - кавычки трёх сортов («ёлочки», „лапки", "прямые") и тире трёх длин —
 *    посетитель наберёт с клавиатуры кита прямой дефис и не найдёт ничего;
 *  - «В.И. Ленин» и «В. И. Ленин» отличаются пробелом после точки.
 * Поэтому и запрос, и поле сводим к одному виду: строчные, ё→е, любые
 * тире и кавычки → пробел, пробелы схлопнуты.
 */
...
/**
 * Совпадение по ЛЮБОМУ из объявленных полей и по КАЖДОМУ слову запроса.
 * Слова И, а не ИЛИ: «ленин петербург» должно сузить, а не расширить, —
 * иначе второе слово увеличивает выдачу, и посетитель, уточняя запрос,
 * получает больше, чем просил.
 * Слова могут приходиться на РАЗНЫЕ поля: «ленин петербург» — автор плюс
 * место. Отсюда склейка полей в одну строку, а не проверка полей по
 * отдельности.
 */
export function matchQuery(item, fields, nq) {
  if (!nq) return true;
  const hay = normQuery(fields.map((f) => item[f]).filter(Boolean).join(" "));
  for (const w of nq.split(" ")) if (w && hay.indexOf(w) < 0) return false;
  return true;
}
...
export function createHint(el, app, gesture, key) {
  ...
  return {
    setLang() { h = attach(); },
    /* show() зовём на выходе из заставки: будящее касание уходит в оверлей
     * ядра, до poke() подсказки не доходит, и сама она не показалась бы.
     * Ядро снимает подавление раньше стоп-функции сцены, поэтому призыв
     * загорается сразу — свежему посетителю он нужнее всех. */
    show() { if (h) h.show(); },
