...
export function createHint(el, app, gesture, key) {
  ...
  return {
     ...
     * show() нужен на выходе: idleTimer кита одноразовый и к моменту
     * заставки уже отработал, а будящее касание уходит в оверлей ядра —
     * poke() не случается, и свежий посетитель, которому призыв нужнее
     * всех, не увидел бы его никогда. */
    hide() { if (h) h.hide(); },
    show() { if (h) h.show(); },
    /* Отодвинуть показ. Нужен на каждом тике заставки: hide() кита с 1.9.x
     * ПЕРЕВЗВОДИТ свой одноразовый 30-секундный таймер (так чинили обратный
     * дефект — подсказка не возвращалась после заставки), поэтому одного
     * hide() на входе мало: через полминуты призыв загорелся бы поверх
     * аттрактора и горел до утра. */
    poke() { if (h) h.poke(); },
