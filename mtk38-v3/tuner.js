...
// API: createTuner({ title, groups:[{title,params:[{key,label,min,max,step,value}]}],
//                    segments:[{key,label,options:[[value,label],…],value}], onChange, collapsed })
...
const CSS = `
.v3tuner{position:fixed;top:14px;left:14px;z-index:8;width:230px;max-height:92vh;overflow:auto;
...
.v3tuner-h{display:flex;justify-content:space-between;align-items:center;padding:8px 12px;
...
.v3tuner-seg{display:flex;gap:4px;margin-top:4px;flex-wrap:wrap}
.v3tuner-seg button{flex:1;min-width:40px;border:1px solid rgba(157,163,168,.3);background:rgba(157,163,168,.12);
  color:#CFD0CF;border-radius:6px;padding:5px 4px;font:inherit;font-size:11px;cursor:pointer}
.v3tuner-seg button.on{background:#D2B773;color:#1a1f23;border-color:#D2B773;font-weight:600}
...
export function createTuner({ title = 'параметры', groups = [], segments = [], onChange, collapsed = false } = {}) {
  ...
  box.innerHTML =
    ...
    segments.map((s) => `<div class="v3tuner-g"><span>${s.label}</span><div class="v3tuner-seg" data-key="${s.key}">` +
      s.options.map(([v, l]) => `<button data-v="${v}"${v === s.value ? ' class="on"' : ''}>${l}</button>`).join('') +
      `</div></div>`).join('') +
  ...
  const setSeg = (key, v) => {
    const seg = box.querySelector(`.v3tuner-seg[data-key="${key}"]`); if (!seg) return;
    seg.querySelectorAll('button').forEach((b) => b.classList.toggle('on', b.dataset.v === String(v)));
  };
  for (const s of segments) {
    const seg = box.querySelector(`.v3tuner-seg[data-key="${s.key}"]`);
    seg.querySelectorAll('button').forEach((btn) => {
      btn.onclick = () => {
        let v = btn.dataset.v; if (v !== '' && !isNaN(+v)) v = +v;   // число, если числовое
        values[s.key] = v; setSeg(s.key, v);
        onChange && onChange(s.key, v, values);
      };
    });
  }
  ...
  box.querySelector('.v3tuner-reset').onclick = () => {
    for (const k in defaults) {
      values[k] = defaults[k];
      const r = box.querySelector('#tr_' + k);
      if (r) { r.value = defaults[k]; box.querySelector('#tv_' + k).textContent = fmt(defaults[k]); }
      else setSeg(k, defaults[k]);
      onChange && onChange(k, defaults[k], values);
    }
  };

  return {
    dom: box, values, get: () => values,
    set: (k, v) => { const r = box.querySelector('#tr_' + k); if (r) { r.value = v; values[k] = v; box.querySelector('#tv_' + k).textContent = fmt(v); } else { values[k] = v; setSeg(k, v); } onChange && onChange(k, v, values); },
    toggle: () => (box.style.display === 'none' ? expand() : collapse()),
  };
}
