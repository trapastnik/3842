...
const RUBRICS = [
  {
    key: "one-off",
    name: "В единственном экземпляре",
    note: "Небесные тела, суда, каналы, вершины, геоглиф в тайге и парк-музей "
      + "в литовском лесу — то, что существует в одном экземпляре.",
    test: (r) => ["космос", "судно", "природа", "вода", "награда"].includes(r.kind)
      || ONE_OFF.test(r.name),
  },
  {
    key: "address",
    name: "Улицы и площади",
    note: "Проспекты, бульвары, переулки. У такого объекта имя и есть он сам: "
      + "снимите табличку — не останется ничего. Основная масса зарубежной части свода.",
    test: (r) => ["улица", "площадь", "проспект", "переулок"].includes(r.kind),
  },
  {
    key: "settlement",
    name: "Города и сёла",
    note: "Посёлки, станицы, районы и целые области, названные именем Ленина. "
      + "Почти всё — бывший Советский Союз, и почти всё переименовано.",
    test: (r) => ["город", "район"].includes(r.kind),
  },
  {
    key: "work",
    name: "Заводы и станции",
    note: "Комбинаты, электростанции, шахты, колхозы, метро и железные дороги. "
      + "Здесь имя — не адрес, а звание: объект работает и под другим названием.",
    test: (r) => ["завод", "электростанция", "колхоз", "транспорт"].includes(r.kind),
  },
  {
    key: "mind",
    name: "Институты и культура",
    note: "Университеты, школы, библиотеки, музеи, театры, стадионы и больницы — "
